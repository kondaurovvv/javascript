# ЦЕЛЬ.

Хочу выучить язык JavaScript с помощью learn.javascript.ru.

## Задачи.

Введение

- [Введение в JavaScript](https://learn.javascript.ru/intro)
- [Справочники и спецификации](https://learn.javascript.ru/manuals-specifications)
- [Редакторы кода](https://learn.javascript.ru/code-editors)
- [Консоль разработчика](https://learn.javascript.ru/devtools)

Основы JavaScript

- [Привет, мир!](https://learn.javascript.ru/hello-world)
- [Структура кода](https://learn.javascript.ru/structure)
- [Строгий режим — "use strict"](https://learn.javascript.ru/strict-mode)
- [Переменные](https://learn.javascript.ru/variables)
- [Типы данных](https://learn.javascript.ru/types)
- [Взаимодействие: alert, prompt, confirm](https://learn.javascript.ru/alert-prompt-confirm)
- [Преобразование типов](https://learn.javascript.ru/type-conversions)
- [Базовые операторы, математика](https://learn.javascript.ru/operators)
- [Операторы сравнения](https://learn.javascript.ru/comparison)
- [Условное ветвление: if, '?'](https://learn.javascript.ru/ifelse)
- [Логические операторы](https://learn.javascript.ru/logical-operators)
- [Операторы нулевого слияния и присваивания: '??', '??='](https://learn.javascript.ru/nullish-operators)
- [Циклы while и for](https://learn.javascript.ru/while-for)
- [Конструкция "switch"](https://learn.javascript.ru/switch)
- [Функции](https://learn.javascript.ru/function-basics)
- [Function Expression](https://learn.javascript.ru/function-expressions)
- [Стрелочные функции, основы](https://learn.javascript.ru/arrow-functions-basics)
- [Особенности JavaScript](https://learn.javascript.ru/javascript-specials)
- [Отладка в браузере](https://learn.javascript.ru/debugging-chrome)
- [Советы по стилю кода](https://learn.javascript.ru/coding-style)
- [Комментарии](https://learn.javascript.ru/comments)
- [Ниндзя-код](https://learn.javascript.ru/ninja-code)
- [Автоматическое тестирование c использованием фреймворка Mocha](https://learn.javascript.ru/testing-mocha)
- [Полифилы](https://learn.javascript.ru/polyfills)

Объекты: основы

- [Объекты](https://learn.javascript.ru/object)
- [Копирование объектов и ссылки](https://learn.javascript.ru/object-copy)

## Философия Agile.

### Четыре ценности Agile-манифеста.

- **ЛЮДИ И ВЗАИМОДЕЙСТВИЯ** важнее процессов и инструментов.
- **РАБОТАЮЩИЙ ПРОДУКТ** важнее точной и подробной документации.
- **СОТРУДНИЧЕСТВО С ЗАКАЗЧИКОМ** важнее условий договора.
- **ГОТОВНОСТЬ К ИЗМЕНЕНИЯМ** важнее следования изначальному плану.

### Двенадцать принципов Agile.

- Удовлетворение потребностей клиента через раннюю и регулярную поставку ценного ПО.
- Приветствие изменений требований, даже на поздних стадиях, для повышения конкурентоспособности заказчика.
- Частый выпуск работающего продукта (раз в пару недель или месяцев).
- Ежедневное сотрудничество разработчиков и представителей бизнеса.
- Работа над проектом мотивированных профессионалов, создание условий, поддержка и доверие.
- Личное общение — самый эффективный способ обмена информацией.
- Работающий продукт — основной показатель прогресса.
- Устойчивый ритм: инвесторы, разработчики и пользователи должны поддерживать постоянную скорость работы.
- Техническое совершенство и качество проектирования повышают гибкость.
- Простота — искусство минимизации лишней работы.
- Лучшие решения рождаются у самоорганизующихся команд.
- Регулярная рефлексия: команда постоянно анализирует, как стать эффективнее, и корректирует работу.

## Фреймворк Scrum.

### Команда (Team):

- Владелец продукта (Product Owner).
- Scrum-мастер.
- Разработчик.

### События (Events):

- Спринт (Sprint): Цикл работы — **2 недели.**
- Планирование спринта (Sprint Planning): Определение целей и задач. Проводится в первый понедельник спринта, длительность — **4 часа**.
- Daily Scrum (Ежедневный скрам/стендап): **15-минутная** встреча для синхронизации, проводится каждый день.
- Обзор спринта (Sprint Review): Демонстрация результатов заказчику. Проводится в последний четверг спринта, длительность — **1 час**.
- Ретроспектива спринта (Sprint Retrospective): Анализ работы команды и улучшение процессов. Проводится в последнюю пятницу спринта, длительность — **1 час**.

### Артефакты (Artifacts):

- Бэклог продукта (Product Backlog): Список всех задач и требований.
- Бэклог спринта (Sprint Backlog): Задачи на текущий спринт.
- Инкремент (Increment): Готовая, работающая часть продукта.
